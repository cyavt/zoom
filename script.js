...
const vm = new Vue({
  ...
  computed: {
    ...
    roomU: function() {
  	this.rooma = `${this.roomId}`.substr(21,13);
      return `${this.rooma}`;
    }
  },
  async mounted() {
  ...
  methods: {
    ...
    joinWithId: async function() {
      ...
      rooms =/^\d*$/.test(roomId);
      if (roomId && rooms == true) {
        this.roomId = 'room-vn-1-QT2VAHEPXN-'+roomId;
        window.location.href = `https://${location.hostname}?room=${this.roomId}`;
      }else{
